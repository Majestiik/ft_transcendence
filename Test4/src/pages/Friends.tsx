...
const Friends = () => {
	...
	useEffect(() => {
		if (shouldUpdate)
		{
			...
			axios.get('http://10.2.12.2:3003/clients').then((ret) => userContext.updateClientsData(ret.data));
			axios.get('http://10.2.12.2:3003/clients').then((ret) => userContext.updateFriendsData(ret.data));
			window.addEventListener("beforeunload", function() {axios.patch('http://10.2.12.2:3003/clients/' + userContext.id, {online: false}/*{name: userContext.name, avatar: userContext.avatar, level: userContext.level, online: false, ingame: userContext.ingame, friends: userContext.friendsData}*/)});
	...
	const addFriend = () => {
		...
		if (find)
		{
			...
			axios.put('http://10.2.12.2:3003/clients/' + userContext.id, {name: userContext.name, avatar: userContext.avatar, level: userContext.level, online: userContext.online, ingame: userContext.ingame, friends: userContext.friendsData});
		}
		else
			alert("Not Found !");
	};
